...
int main(void) {
	...
	printf(
			"Enter a command [ls= left scan, rs= right scan, ws= weight scale, lo =left open, ro=right open, lc = left closed, rc = right closed , gru = guard right unlock, grl = guard right lock, gll=guard left lock, glu = guard left unlock, exit]\n");
	// Phase 2
	while (1) {
...
void getUsrPrompt(char input[4], Person* person) {
	...
	if (strcmp(input, inMessage[LS_INPUT]) == 0) {
		...
		getUsrID(person);
	} else if (strncmp(input, inMessage[RS_INPUT], strlen(inMessage[RS_INPUT]))
			== 0) {
		person->state = RS_STATE;
		getUsrID(person);
	} else if (strncmp(input, inMessage[WS_INPUT], strlen(inMessage[WS_INPUT]))
			== 0) {
		person->state = WS_STATE;
		getUsrWeight(person);
	} else if (strncmp(input, inMessage[LO_INPUT], strlen(inMessage[LO_INPUT]))
			== 0) {
		person->state = LO_STATE;
	} else if (strncmp(input, inMessage[RO_INPUT], strlen(inMessage[RO_INPUT]))
			== 0) {
		person->state = RO_STATE;
	} else if (strncmp(input, inMessage[LC_INPUT], strlen(inMessage[LC_INPUT]))
			== 0) {
		person->state = LC_STATE;
	} else if (strncmp(input, inMessage[RC_INPUT], strlen(inMessage[RC_INPUT]))
			== 0) {
		person->state = RC_STATE;
	} else if (strncmp(input, inMessage[GLU_INPUT],
			strlen(inMessage[GLU_INPUT])) == 0) {
		person->state = GLU_STATE;
	} else if (strncmp(input, inMessage[GRU_INPUT],
			strlen(inMessage[GRU_INPUT])) == 0) {
		person->state = GRU_STATE;
	} else if (strncmp(input, inMessage[GLL_INPUT],
			strlen(inMessage[GLL_INPUT])) == 0) {
		person->state = GLL_STATE;
	} else if (strncmp(input, inMessage[GRL_INPUT],
			strlen(inMessage[GRL_INPUT])) == 0) {
		person->state = GRL_STATE;
	} else if (strncmp(input, inMessage[EXIT_INPUT],
			strlen(inMessage[EXIT_INPUT])) == 0) {
